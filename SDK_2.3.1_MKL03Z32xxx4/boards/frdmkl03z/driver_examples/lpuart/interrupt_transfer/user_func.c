...
#define CTL_SET {GPIO_PinWrite(GPIOB, PIN_CTL, GPIO_H);}
#define CTL_CLR {GPIO_PinWrite(GPIOB, PIN_CTL, GPIO_L);}
#define SYN_SET {GPIO_PinWrite(GPIOB, PIN_SYN, GPIO_H);}
#define SYN_CLR {GPIO_PinWrite(GPIOB, PIN_SYN, GPIO_L);}
...
//    }
    ...
    config.pinDirection = kGPIO_DigitalOutput;
	config.outputLogic = GPIO_L;
	GPIO_PinInit(GPIOB, PIN_CTL, &config);

#ifdef SYN_TX
    // OUT SYN
    config.pinDirection = kGPIO_DigitalOutput;
	config.outputLogic = GPIO_L;
	GPIO_PinInit(GPIOB, PIN_SYN, &config);
#endif
...
//    SYN_SET;
//    SYN_CLR;
    u8 mod = GPIO_PinRead(GPIOB, PIN_SYN);
    mod = GPIO_PinRead(GPIOB, PIN_SYN);
...
// 单位为微秒，*2 为保持时长
//#define LPTMR_USEC_COUNT 2500U  // 1s
//#define LPTMR_USEC_COUNT 5000000U
#define LPTMR_USEC_COUNT 1000000U//2s
//#define TIMER_DEBUG
#ifdef TIMER_DEBUG
...
#ifdef SYN_TX
    u8 mod = GPIO_PinRead(GPIOB, PIN_MOD);
    if(mod)
#else
    if(1)
#endif
